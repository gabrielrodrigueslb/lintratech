...
const projects = [
  {
    ...
    description: "A comprehensive platform for managing digital products."
  },
  {
    id: 7,
    ...
    <section id="portfolio" className="py-24 bg-background relative">
      <div className="container">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6 reveal">
          <div>
            <h2 className="font-mono text-primary tracking-widest mb-2 text-sm uppercase">{t('portfolio.subtitle')}</h2>
            <h3 className="font-display font-bold text-4xl md:text-5xl">{t('portfolio.title')}</h3>
          </div>
          <Link href="/catalog">
            <Button variant="outline" className="rounded-none border-primary text-primary hover:bg-primary hover:text-black font-mono gap-2 group">
              {t('portfolio.viewAll')}
              ...
              <Card className="bg-card border-border overflow-hidden group rounded-none hover:border-primary transition-colors duration-300 cursor-pointer h-full flex flex-col">
                <div className="relative aspect-video overflow-hidden">
                  ...
                  <div className="absolute top-4 right-4 z-20 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity translate-y-2 group-hover:translate-y-0 duration-300">
                    <Button size="icon" variant="secondary" className="rounded-full h-10 w-10 bg-background/80 text-foreground hover:bg-primary hover:text-black border border-border">
                ...
                <CardHeader className="pb-2">
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      <h4 className="font-mono text-xs text-primary tracking-wider uppercase">{project.category}</h4>
                      <h3 className="font-display font-bold text-xl md:text-2xl group-hover:text-primary transition-colors">{project.title}</h3>
                    </div>
                  </div>
                </CardHeader>
                ...
                <CardContent className="grow flex flex-col justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map((tag) => (
                      <Badge key={tag} variant="outline" className="rounded-sm border-border text-xs font-mono text-muted-foreground bg-muted/50">
                        {tag}
                      </Badge>
                    ))}
                  </div>
